...
const disguiseText = (text: string | number | undefined, type: 'name' | 'mrn' | 'text' | 'age' | 'gender', isDisguised: boolean, patient?: { name: string, mrn?: string }) => {
  ...
  if (type === 'text') {
    ...
    // Phone numbers (International, local, and various formats)
    result = result.replace(/\b(?:\+?\d{1,3}[- ]?)?\(?\d{2,4}\)?[- ]?\d{3,4}[- ]?\d{4,6}\b/g, '[PHONE]');
    
    // Emails
    result = result.replace(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g, '[EMAIL]');
    
    // SSN / National IDs / CNIC / Tax Numbers (NTN)
    // SSN: XXX-XX-XXXX
    result = result.replace(/\b\d{3}-\d{2}-\d{4}\b/g, '[ID]');
    // CNIC: XXXXX-XXXXXXX-X
    result = result.replace(/\b\d{5}-\d{7}-\d{1}\b/g, '[CNIC]');
    // Generic Tax/National ID patterns (often 7-12 digits with optional hyphens)
    result = result.replace(/\b\d{7,10}-\d{1}\b/g, '[TAX_ID]'); // Common NTN pattern
    result = result.replace(/\b\d{9,12}\b/g, (match) => {
      // Only mask long number strings that aren't likely to be clinical values (e.g., HR, BP)
      return match.length >= 9 ? '[IDENTIFIER]' : match;
    });

    // Addresses (Street, Road, House, Block patterns)
    result = result.replace(/\b(House|H|Apt|Flat|No|Block|Plot|Sector|Street|St|Road|Rd|Colony|Area)\.?\s*#?\d+[A-Z]?\b/gi, '[ADDRESS]');
    result = result.replace(/\b\d+\s+(Street|St|Road|Rd|Avenue|Ave|Drive|Dr|Lane|Ln|Court|Ct|Circle|Cir)\b/gi, '[ADDRESS]');
    
    // Specific sensitive keywords
    result = result.replace(/\b(lives in|resident of|located at)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b/gi, '$1 [LOCATION]');
...
Generated by AuraMD Ambient Pilot
  ...
  const handleGoogleAuth = async () => {
    // Google Auth removed by user request
    ...
    return (
      <div className="min-h-screen bg-[#121417] flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        >
          ...
          >
            <Logo id="main" className="w-44 h-44" glow={true} />
          </motion.div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-10 w-full text-left">
            {[
              { icon: <ShieldCheck className="w-4 h-4" />, label: "Patient Confidentiality", desc: "Assured & Encrypted", color: "text-emerald-400" },
          ...
          <div className="w-full space-y-4">
            <form onSubmit={handleEmailAuth} className="space-y-4 w-full">
              {!isResetMode && (
                <>
                  <div className="relative">
                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#a1aab5]" />
                    <input
                      type="email"
                      value={authEmail}
                      ...
                      required
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-[#a1aab5] focus:outline-none focus:border-cyan-500/50 transition-all"
                    />
                  </div>
                  <div className="relative">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#a1aab5]" />
                    <input
                      type="password"
                      value={authPassword}
                  ...
                  {isSignUp && (
                    ...
                    >
                      <div className="relative">
                        <Stethoscope className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#a1aab5]" />
                        <select
                          value={authRole}
                          onChange={(e) => setAuthRole(e.target.value)}
                          className="w-full bg-[#1e2226] border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white focus:outline-none focus:border-cyan-500/50 appearance-none transition-all cursor-pointer"
                        >
                      ...
                      <div className="relative">
                        <Building className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#a1aab5]" />
                        <input
                          type="text"
                          value={authInstitute}
                          onChange={(e) => setAuthInstitute(e.target.value)}
              ...
              >
                ...
                {isResetMode ? 'Send Reset Link' : isSignUp ? 'Create Clinical Account' : 'Sign In to Nexus'}
              </motion.button>
              ...
              <div className="flex justify-between px-2">
                ...
                <button
                  ...
                  className="text-[10px] font-bold text-[#a1aab5] uppercase tracking-widest hover:text-white transition-colors"
                >
            ...
            {resetSent && (
              <motion.p 
                initial={{ opacity: 0, height: 0 }}
                animate={{ opacity: 1, height: 'auto' }}
                className="text-emerald-400 text-[11px] mt-4 bg-emerald-500/10 py-3 px-4 rounded-xl border border-emerald-500/20 text-left flex items-center gap-3 leading-tight"
              >
                        ...
                        <div className="pt-2 border-t border-[#2c3137]">
                          ...
                          <div className="mt-3 text-[9px] text-[#a1aab5] text-center space-y-1 opacity-60">
                            ...
                            <p className="flex items-center justify-center gap-1">
                              <ShieldCheck className="w-2.5 h-2.5 text-emerald-500" /> Patient Confidentiality
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                <div className="mt-4 text-[10px] text-[#a1aab5] text-center space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-emerald-500/80 font-medium flex items-center justify-center gap-1">
                      <ShieldCheck className="w-3 h-3" /> Patient Confidentiality
                    </p>
                  </div>
                </div>
              </>
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-1.5 sm:gap-4 overflow-x-auto no-scrollbar py-2 relative z-10">
          ...
          <button
            ...
            title={isDisguised ? "Privacy Mask Active" : "Activate Privacy Mask"}
          >
            {isDisguised ? <ShieldCheck className="w-3.5 h-3.5" /> : <ShieldOff className="w-3.5 h-3.5" />}
            <span>Confidentiality Mask</span>
          </button>
        ...
        {!activePatient ? (
          currentView === 'pricing' ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 pt-24 lg:p-12 lg:pt-28 overflow-y-auto relative">
              ...
              <Logo id="main" className="w-56 h-56 mb-8" showText={true} />
                ...
                <div className="text-center space-y-4">
                  ...
                  <div className="flex flex-wrap justify-center gap-4 py-2">
                    {[
                      ...
                      { icon: <Lock className="w-4 h-4" />, label: "Patient Confidentiality" }
                    ].map((feature, idx) => (
                      <motion.div 
                        key={idx}
                ...
                <div className="pt-8 mt-8 border-t border-[#2c3137]/50 w-full">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    ...
                    <div className="flex flex-col items-center text-center p-5 rounded-2xl bg-[#1e2226]/30 border border-[#2c3137]/30 hover:bg-[#1e2226] transition-colors">
                      ...
                      <p className="text-xs text-[#a1aab5]">Strict Privacy Mode</p>
                    </div>
                    <div className="flex flex-col items-center text-center p-5 rounded-2xl bg-[#1e2226]/30 border border-[#2c3137]/30 hover:bg-[#1e2226] transition-colors">
                        ...
                        <div className="text-[#b3b9c1] leading-relaxed">
                          ...
                          {reasoningResult.references && reasoningResult.references.length > 0 && (
                            <div className="mt-6 pt-6 border-t border-[#2c3137]">
                              <h5 className="text-[10px] font-bold text-[#a1aab5] uppercase tracking-widest mb-3">Key Evidence Citations</h5>
                              <ul className="space-y-2">
                                {reasoningResult.references.slice(0, 3).map((ref, i) => (
                                  <li key={i} className="text-xs text-[#a1aab5] flex items-start gap-2">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            ...
            >
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div className="space-y-2">
                  <h2 className="text-2xl font-bold text-white">Strict Privacy Mode</h2>
                ...
                <div className="flex gap-3">
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setTempPrivacyKey('');
                      setPrivacyKeyError(null);
                      alert('Strict Privacy Mode Enabled. New data will be End-to-End Encrypted.');
                    }}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Directional Guidance - Primary Focus */}
          ...
          {latestState?.references && latestState.references.length > 0 && (
            <section className="space-y-6 mt-12">
              <div className="flex items-center gap-3 px-1">
                <BookOpen className="w-6 h-6 text-[#a1aab5]" />
                <h3 className="text-[10px] font-bold text-[#a1aab5] uppercase tracking-widest">Evidence References</h3>
              </div>
              <div className="bg-[#1e2226]/50 border border-[#2c3137] rounded-3xl p-8">
                <ul className="space-y-3">
                  {latestState.references.map((ref, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-[#a1aab5]">
                      <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-cyan-500 shrink-0" />
                      <div>
                        <span className="text-[#ffffff] font-medium">{ref.citation}</span>
                        {ref.url && (
                          <a 
                            href={ref.url} 
                            target="_blank" 
